...
1. **Not bootstrapped** (`.pn/settings.json` missing or required values unresolved): suggest `/pn-po:bootstrap-project` and stop.
2. **No Jira key parsed from branch**: suggest `/pn-po:new-capture-idea` (if no idea-brief exists yet for this work) and `/pn-po:refine-capture-context` (if an epic key was provided manually). Stop.
...
4. **Phase-driven suggestions** (no drift, Jira reachable): map the current `PN Phase` to the canonical next skill(s):
   ...
   |----------|----------------------|
   | new | `/pn-po:refine-capture-context` |
   | refine | `/pn-po:refine-grill-requirements`, `/pn-po:refine-draft-epic` |
   | estimate | `/pn-po:estimate-write-prd`, `/pn-po:estimate-check-ticket` |
   | approval | `/pn-po:todo-plan-phases` |
   | todo | `/pn-po:progress-implement` |
   | progress | `/pn-po:progress-implement` (continue), `/pn-po:review-pr` (after push) |
   | review | `/pn-po:review-pr`, `/pn-po:review-evaluate-code` |
   | handover | `/pn-po:handover-release-notes`, `/pn-po:handover-gate` |
   | deploy | `/pn-po:release-readiness-gate` |
   | release-ready | `/pn-po:release-readiness-gate` |
   | released | (none; story is done) |

5. **Scratchpad signals**: if the scratchpad contains an "Open questions" or "Blockers" section with non-empty bullets, prepend `/pn-po:refine-grill-requirements` to the suggestions regardless of phase.
