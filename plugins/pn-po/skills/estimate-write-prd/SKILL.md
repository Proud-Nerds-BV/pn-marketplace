...
description: Produces docs/pn/prd/<EPIC>.md (a full PRD with frontmatter linking to the Jira epic) and creates one Jira story per outline-story under the epic, each populated with Acceptance Criteria, Expected Scope, story points, and PN Actor. All stories link back to the PRD in their description. Idempotent: re-runs update existing stories rather than duplicating them. Use when the user invokes /pn-po:estimate-write-prd, asks to write the PRD, or when pn_phase on the epic is estimate and no PRD exists yet.
