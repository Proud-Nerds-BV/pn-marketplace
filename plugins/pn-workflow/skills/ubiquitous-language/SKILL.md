...
|------------|--------|-------------|
| `add` | `/pn-workflow:ubiquitous-language add <term> <definition>` | Adds a new entry; fails if the term already exists |
| `update` | `/pn-workflow:ubiquitous-language update <term>` | Opens an interactive edit loop for an existing entry; fails if the term does not exist |
| `audit` | `/pn-workflow:ubiquitous-language audit` | Suggests terms from code-knowledge summaries that are not in the glossary; the user confirms which to add |
