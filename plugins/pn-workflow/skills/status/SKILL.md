...
1. **Not bootstrapped** (`.pn/settings.json` missing or required values unresolved): suggest `/pn-workflow:bootstrap-project` and stop.
2. **No Jira key parsed from branch**: suggest `/pn-workflow:new-capture-idea` (if no idea-brief exists yet for this work) and `/pn-workflow:refine-capture-context` (if an epic key was provided manually). Stop.
...
4. **Phase-driven suggestions** (no drift, Jira reachable): map the current `PN Phase` to the canonical next skill(s):
   ...
   |----------|----------------------|
   | new | `/pn-workflow:refine-capture-context` |
   | refine | `/pn-workflow:refine-grill-requirements`, `/pn-workflow:refine-draft-epic` |
   | estimate | `/pn-workflow:estimate-write-prd`, `/pn-workflow:estimate-check-ticket` |
   | approval | `/pn-workflow:todo-plan-phases` |
   | todo | `/pn-workflow:progress-implement` |
   | progress | `/pn-workflow:progress-implement` (continue), `/pn-workflow:review-pr` (after push) |
   | review | `/pn-workflow:review-pr`, `/pn-workflow:review-evaluate-code` |
   | handover | `/pn-workflow:handover-release-notes`, `/pn-workflow:handover-gate` |
   | deploy | `/pn-workflow:release-readiness-gate` |
   | release-ready | `/pn-workflow:release-readiness-gate` |
   | released | (none; story is done) |

5. **Scratchpad signals**: if the scratchpad contains an "Open questions" or "Blockers" section with non-empty bullets, prepend `/pn-workflow:refine-grill-requirements` to the suggestions regardless of phase.
