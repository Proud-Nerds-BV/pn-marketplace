...
1. **Not bootstrapped** (`.pn/settings.json` missing or required values unresolved): suggest `/pn-release:bootstrap-project` and stop.
2. **No Jira key parsed from branch**: suggest `/pn-release:new-capture-idea` (if no idea-brief exists yet for this work) and `/pn-release:refine-capture-context` (if an epic key was provided manually). Stop.
...
4. **Phase-driven suggestions** (no drift, Jira reachable): map the current `PN Phase` to the canonical next skill(s):
   ...
   |----------|----------------------|
   | new | `/pn-release:refine-capture-context` |
   | refine | `/pn-release:refine-grill-requirements`, `/pn-release:refine-draft-epic` |
   | estimate | `/pn-release:estimate-write-prd`, `/pn-release:estimate-check-ticket` |
   | approval | `/pn-release:todo-plan-phases` |
   | todo | `/pn-release:progress-implement` |
   | progress | `/pn-release:progress-implement` (continue), `/pn-release:review-pr` (after push) |
   | review | `/pn-release:review-pr`, `/pn-release:review-evaluate-code` |
   | handover | `/pn-release:handover-release-notes`, `/pn-release:handover-gate` |
   | deploy | `/pn-release:release-readiness-gate` |
   | release-ready | `/pn-release:release-readiness-gate` |
   | released | (none; story is done) |

5. **Scratchpad signals**: if the scratchpad contains an "Open questions" or "Blockers" section with non-empty bullets, prepend `/pn-release:refine-grill-requirements` to the suggestions regardless of phase.
