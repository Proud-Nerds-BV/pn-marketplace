...
1. **Not bootstrapped** (`.pn/settings.json` missing or required values unresolved): suggest `/pn-dev:bootstrap-project` and stop.
2. **No Jira key parsed from branch**: suggest `/pn-dev:new-capture-idea` (if no idea-brief exists yet for this work) and `/pn-dev:refine-capture-context` (if an epic key was provided manually). Stop.
...
4. **Phase-driven suggestions** (no drift, Jira reachable): map the current `PN Phase` to the canonical next skill(s):
   ...
   |----------|----------------------|
   | new | `/pn-dev:refine-capture-context` |
   | refine | `/pn-dev:refine-grill-requirements`, `/pn-dev:refine-draft-epic` |
   | estimate | `/pn-dev:estimate-write-prd`, `/pn-dev:estimate-check-ticket` |
   | approval | `/pn-dev:todo-plan-phases` |
   | todo | `/pn-dev:progress-implement` |
   | progress | `/pn-dev:progress-implement` (continue), `/pn-dev:review-pr` (after push) |
   | review | `/pn-dev:review-pr`, `/pn-dev:review-evaluate-code` |
   | handover | `/pn-dev:handover-release-notes`, `/pn-dev:handover-gate` |
   | deploy | `/pn-dev:release-readiness-gate` |
   | release-ready | `/pn-dev:release-readiness-gate` |
   | released | (none; story is done) |

5. **Scratchpad signals**: if the scratchpad contains an "Open questions" or "Blockers" section with non-empty bullets, prepend `/pn-dev:refine-grill-requirements` to the suggestions regardless of phase.
