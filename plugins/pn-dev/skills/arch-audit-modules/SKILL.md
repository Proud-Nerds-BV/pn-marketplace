...
description: Runs a five-dimension architectural scorecard across all in-scope modules and produces docs/pn/audits/<DATE>-modules.md. Trigger when the user runs /pn-dev:arch-audit-modules, asks to "audit the architecture", "score the modules", "check for architectural drift", or "run a module audit". Each dimension (depth, abstraction, type-safety, imports, autonomy) is evaluated by a dedicated sub-agent. Stage 2 creates a single RFC story in Jira for modules that score below threshold. Configurable thresholds in .pn/settings.json.
